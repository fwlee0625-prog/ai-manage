...
export class ProvidersRepository {
  ...
  async list(tool?: AiTool): Promise<AiProviderProfile[]> {
    const rows = await this.manage.all<ProviderRow>(`SELECT * FROM providers ${tool ? `WHERE tool = ${sqlString(tool)}` : ''} ORDER BY sort_index ASC, updated_at DESC, name ASC;`);
  ...
  async save(provider: AiProviderProfile): Promise<void> {
    await this.manage.exec(`
      INSERT INTO providers (id, tool, name, provider_type, endpoint, api_protocol, default_model, reasoning_effort, auth_mode, account_id, credential_id, sort_index, metadata_json, created_at, updated_at)
      VALUES (${sqlString(provider.id)}, ${sqlString(provider.tool)}, ${sqlString(provider.name)}, ${sqlString(provider.providerType)}, ${sqlString(provider.endpoint)}, ${sqlString(provider.apiProtocol)}, ${sqlString(provider.defaultModel)}, ${sqlString(provider.reasoningEffort)}, ${sqlString(provider.authMode)}, ${sqlString(provider.accountId)}, ${sqlString(provider.credentialId)}, ${Number(provider.sortIndex ?? 0)}, ${sqlString(JSON.stringify(provider.metadata || {}))}, ${sqlString(provider.createdAt)}, ${sqlString(provider.updatedAt)})
      ON CONFLICT(id) DO UPDATE SET tool=excluded.tool, name=excluded.name, provider_type=excluded.provider_type, endpoint=excluded.endpoint, api_protocol=excluded.api_protocol, default_model=excluded.default_model, reasoning_effort=excluded.reasoning_effort, auth_mode=excluded.auth_mode, account_id=excluded.account_id, credential_id=excluded.credential_id, sort_index=excluded.sort_index, metadata_json=excluded.metadata_json, updated_at=excluded.updated_at;
    `);
  }

  /** Persists an explicit provider order for one tool. */
  async reorder(providerIds: string[]): Promise<void> {
    await this.manage.transaction(providerIds.map((id, index) =>
      `UPDATE providers SET sort_index=${index} WHERE id=${sqlString(id)};`,
    ));
  }
